...
public static class ExecutionOrderAttributeEditor
{
	...
	static List<ScriptExecutionOrderDependency> GetExecutionOrderDependencies(Dictionary<Type, MonoScript> types)
	{
		...
		foreach (var kvp in types)
		{
			...
			bool hasExecutionOrderAttribute = Attribute.IsDefined(type, typeof(ScriptExecutionOrderAttribute));
			bool hasDependsOnAttribute = Attribute.IsDefined(type, typeof(ScriptExecutionOrderDependsOnAttribute));

			if (hasDependsOnAttribute)
			{
				if (hasExecutionOrderAttribute)
				{
					Debug.LogError($"Script {script.name} has both [ScriptExecutionOrder] and [ScriptExecutionOrderDependsOn] attributes. Ignoring the [ScriptExecutionOrderDependsOn] attribute.", script);
					continue;
				}

				var attributes = (ScriptExecutionOrderDependsOnAttribute[])Attribute.GetCustomAttributes(type, typeof(ScriptExecutionOrderDependsOnAttribute));
				foreach (var attribute in attributes)
				{
					if (attribute.orderIncrease < 0)
					{
						Debug.LogError($"Script {script.name} has an [ScriptExecutionOrderDependsOn] attribute with a negative orderIncrease.", script);
						continue;
					}
					...
					{
						Debug.LogError(
                            $"Script {script.name} has an [ScriptExecutionOrderDependsOn] attribute with targetScript={attribute.targetType.Name} which is not a MonoBehaviour nor a ScriptableObject. Ignoring this [ScriptExecutionOrderDependsOn] attribute.", script);
						continue;
					}
	...
	static List<ScriptExecutionOrderDefinition> GetExecutionOrderDefinitions(Dictionary<Type, MonoScript> types)
	{
		...
		foreach (var kvp in types)
		{
			var type = kvp.Key;
			var script = kvp.Value;
			if (Attribute.IsDefined(type, typeof(ScriptExecutionOrderAttribute)))
			{
				var attribute = (ScriptExecutionOrderAttribute)Attribute.GetCustomAttribute(type, typeof(ScriptExecutionOrderAttribute));
